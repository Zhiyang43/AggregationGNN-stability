...
class SelectionGNN(nn.Module):
    ...
    def forward(self, x):
        ...
        # forward function.
        output, G_output = self.splitForward(x)
        
        return output, G_output
...
class AggregationGNN(nn.Module):
    ...
    def __init__(self,
                 ...
                 nonlinearity,
                 # Pooling
                 poolingFunction, poolingSize, nSelectedNodes,
                 # MLP in the end
                 dimLayersMLP,
                 # Structure
    ...
    def changeS_forward(self, S_new, x):
        assert len(x.shape) == 3
        ...
        delta = np.zeros([self.E, S_new.shape[1], self.nNodes]) # E x N x nNodes
        for n in range(self.nNodes):
            delta[:, n, n] = 1. # E x N x nNodes
        # And create the place where to store all of this
        SN_new = delta.copy().reshape([self.E, 1, S_new.shape[1], self.nNodes])
        for k in range(1, self.maxN):
            delta = S_new @ delta # E x N x nNodes
            SN_new = np.concatenate((SN_new,
                                 delta\
                              .reshape([self.E, 1, S_new.shape[1], self.nNodes])),
                                axis = 1) # E x k x N x nNodes
        # Now, we have constructed the matrix E x maxN x N x nNodes, but we want
        # is that signal, when multiplied by this matrix, constructs the vector
        # z for each of the nNodes. This vector z is a map between the N-vector
        ...
        # multiplying by the left. Therefore, we want a N x maxN matrix. So we
        # reshape the dimensions
        SN_new = SN_new.transpose(3, 0, 2, 1) # nNodes x E x N x maxN
        # This matrix SN just needs to multiply the incoming x to obtain the
        # aggregated vector. And that's it.
        SN_new = torch.tensor(SN_new)

        SN_new = SN_new.reshape([1, nNodes, E, N, maxN])
        z = torch.matmul(x, SN_new) # B x nNodes x E x F x maxN
        ...
        y_out = self.ConvLayers(z)
        # Flatten the output
        y = y_out.reshape([B * self.nNodes, self.F[-1] * self.N[-1] * self.E])
        ...
        if nNodes == 1 or len(self.dimLayersAggMLP) > 0:
            yflat = y.reshape([B, y.shape[1] * nNodes])
        yMLP = self.AggMLP(yflat)
        # And now we're done
        return yMLP, y_out
